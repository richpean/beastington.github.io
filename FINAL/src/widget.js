...
(function () {
	...
	_each(widgets, function (widget) {
		...
		if( checkDefault("css", widget, d) && !document.getElementById(c + '-css') ){
			...
			css.setAttribute('href', 'src/widget.css');
		...
		var submitText = widget.getAttribute("data-submit-text") || "Let's Get Started";
		
		form.innerHTML =  widgetInner + '<div class="input-field col s6"><input id="icon_prefix" type="email" class="email validate" name="email"><label for="icon_prefix">Email</label></div><div class="response">e</div><div class="waves-effect waves-light btn-large"><input type="submit" value="' + submitText  + '"></div>';
		widget.appendChild(form);

		var messages = {
			"Your request cannot be processed." : widget.getAttribute("data-message-unprocessed") || "An error occurred. Please contact us to subscribe.",
			...
			"You have subscribed to this Marketing Email." : widget.getAttribute("data-message-success") || "Thanks"
    }
      ...
      dispatchCustomEvent(widget, submitEvent);
			...
			console.log('right before send request');

			console.log($('#icon_telephone').val() == '');

			if ($('#name').val() == '') {
				$('.response').html('You forgot to add your name!');
				$('.response').addClass('error');
				return;
			}

			if ($('#age').val() == '') {
				$('.response').html('Don\'t forget your age!');
				$('.response').addClass('error');
				return;
			}
			if (age >= 0.001 && age <= 0.009) {
  			// something
			}

			if ($('#zipcode').val() == '') {
				$('.response').html('Your zipcode can make this super awesome!');
				$('.response').addClass('error');
				return;
			}
  ...
  return this.optional(element) || !!value.trim().match(/^\d{5}(?:[-\s]\d{4})?$/);
			...
			//if ($('input[name=e]:checked').length == 0) {
				//$('.response').html('Tell us your salary.');
				//$('.response').addClass('error');
				//return;
			//}

			if ($('.email').val() == '') {
				$('.response').html('Your email is important to make this work!');
				$('.response').addClass('error');
				return;
			}
			...
			sendRequest('sendgrid.com/newsletter/addRecipientFromWidget', function (req) {
				...
				if(checkDefault("messages", widget, d)){
					...
					responseDiv.innerHTML = messages[responseEventData.message]
						...
						$( "#thank-you-screen" ).delay( 100 ).fadeIn( 900, function() {
    					// Animation complete
						});
						$( "span#theform" ).fadeOut( 100, function() {
    					//Animation complete
						});
...
$( "#thanks-close" ).click(function() {
  $( "#sample-page" ).fadeOut( "slow", function() {
    					// Animation complete
	});
});

$( "a#show-sample" ).click(function() {
	event.preventDefault();
  $( "#sample-page" ).fadeIn( "slow", function() {
    					// Animation complete
	});
});
